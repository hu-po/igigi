...
def _loop():
    ...
    while True:
        ...
        if state.get("reply", None) is not None:
            ...
            tasks.append(Task("set_servos", servos.set_servos(state["reply"]), HPARAMS["set_servo_timeout"]))
        else:
            tasks.append(Task("set_servos", servos.set_servos("forward"), HPARAMS["set_servo_timeout"]))
